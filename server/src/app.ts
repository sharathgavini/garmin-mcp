...
export function createApp(
  ...
  function registerTools(server: McpServer) {
    const toolDescriptions: Record<ToolName, string> = {
      ...
      get_range_summary: "Return compact sleep, HRV, stress, body battery, activity, and recovery trends for a recent date range. Dates use YYYY-MM-DD. For single-day queries, provide only start_date; end_date is optional and defaults to start_date. Optional days means last N days ending today and takes precedence over explicit dates. Reads latest/recent data only. For historical ranges beyond latest coverage, use get_archive_range_summary.",
      ...
      get_system_status: "Return agent-facing Garmin MCP system status: latest sync, archive backfill status, history coverage, available datasets, auth mode summary, and warnings for stale/date-only/missing stream data. Optional include_completeness adds a cheap manifest-derived raw-present/normalized-missing summary. Call this before trusting recovery or historical analysis.",
      ...
      get_activity_streams: "Return full Garmin time-series streams for a specific activity from latest or archive storage, with optional field filtering, pedaling_only cadence filtering, or explicit downsampling.",
      ...
      get_archive_range_summary: "Read partitioned Garmin archive data for a date range and return coverage, missing-data warnings, activity volume, and health trend summaries. Dates use YYYY-MM-DD. For single-day queries, provide only start_date; end_date is optional and defaults to start_date. Optional days means last N days ending today and takes precedence over explicit dates.",
      ...
      get_health_metrics_by_date_range: "Read partitioned Garmin archive daily, sleep, HRV, stress, and body battery records with coverage warnings. Dates use YYYY-MM-DD. For single-day queries, provide only start_date; end_date is optional and defaults to start_date. Optional days means last N days ending today and takes precedence over explicit dates.",
